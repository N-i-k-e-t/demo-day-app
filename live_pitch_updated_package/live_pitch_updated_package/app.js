(() => {
  ...
  function broadcast(type='STATE_SYNC') { saveState(); if (bc) bc.postMessage({ type, state }); }
  ...
  function submitResponse(choice) {
    ...
    // Demo implementation mirrors the production contract: insert-only response.
  ...
  function renderHeader() {
    return `<header class="phone-header">
      ...
      <div style="display:flex;align-items:center;gap:8px"><span class="live-pill">● Live</span><button class="phone-menu" aria-label="Menu">☰</button></div>
    </header>`;
  }
